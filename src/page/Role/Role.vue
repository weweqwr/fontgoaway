          ...
          <div class="authority-content">
            <div class="list-content show">
              <div class="offcial-table tr-border margin-big-top clearfix">
                ...
                <div class="tr clearfix border-bottom-none" v-for="item in Info.roles">
                  <div class="td w20">
                    {{item.roleId}}
                  </div>
                  <div class="td w20">
                  ...
                  <div class="td w30">
                    <div v-for="(pernames,i) in item.perNames" >
                        {{pernames}}
                        <a class="button-word2 btn_ajax_confirm" v-if="flag==0 && item.perIds[i]!=1" v-on:click="deleteRolePermissionByRoleId(item.roleId,item.perIds[i])">删除</a>
                        <a class="button-word2 btn_ajax_confirm" v-if="flag==1" v-on:click="addRolePermission(item.roleId,item.perIds[i])">添加</a>
                    </div>
                  </div>
                  ...
                  <div class="td w15">
                    <a class="button btn-red text-white"  v-if="item.state==1" v-on:click="handleRoleState(item.roleId,0)">禁用</a>
                    <a class="button bg-green-deep" v-if="item.state==0" v-on:click="handleRoleState(item.roleId,1)">启动</a>
                    <a class="button btn-orange text-white" target="_blank" v-on:click="resetCurPage(),baseInfo(item.roleId)">添加</a>
                  </div>
                </div>
              </div>
            </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            baseInfo: function (roleId) {
                var that=this;
                var curPage = that.curPage;
                that.$axios({
                    method: "post",
                    url: "http://47.102.97.234:9090/queryAllRole",
                    data: {
                        curPage: curPage,
                        roleId:roleId
                    },
            ...
            addRolePermission:function (roleId,perId) {
                var that = this;
                var roleId=roleId;
                var perId=perId;
                that.$axios({
                    method: "post",
                    url: "http://47.102.97.234:9090/addRolePermission",
                    data: {
                        roleId:roleId,
                        perId:perId
                    },
