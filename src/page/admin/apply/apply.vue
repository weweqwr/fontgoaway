        ...
        <div class="authority">
          <div class="authority-head">
            <div class="manage-head">
              <h6 class="layout padding-left manage-head-con">管理员申请
            ...
            <div class="list-content show">
              <div class="offcial-table tr-border margin-big-top clearfix">
                <div class="tr-th clearfix">
                  <div id="controller" style="width: 100%;">
                    ...
                    <div class="fr" style="padding-left: 5px;">
                      <input type="search" placeholder="管理员账号"
                ...
                <div class="tr clearfix border-bottom-none" v-for="item in Info.admins">
                  ...
                  <div class="td w15" v-if="item.verify==1">
                    已审核
                  </div>
                  <div class="td w15" v-if="item.verify==0">
                    未审核
                  </div>
                  <div class="td w15">
                    <a class="button btn-red text-white" v-on:click="handleVertity(item.id,1)">通过</a>
                    <a class="button bg-green-deep" v-on:click="handleVertity(item.id,0)">拒绝</a>
                  </div>
                </div>
              </div>
            </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            baseInfo: function (search) {
                var that = this;
                var curPage = that.curPage;
                var account = search;
                that.$axios({
                    method: "post",
                    url: "http://47.102.97.234:9090/queryAllAdmin",
                    data: {
                        verify: 0,
                        account: account,
                        curPage: curPage
            ...
            handleVertity: function (id,state) {
                var that = this;
                var id=id;
                var state=state;
                ...
                that.$axios({
                    method: "post",
                    url: "http://47.102.97.234:9090/handleVertity",
                    data: {
                        id:id,
                        state:state
                    },
