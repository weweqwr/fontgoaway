        ...
        <div class="authority">
          <div class="authority-head">
            <div class="manage-head">
              <h6 class="layout padding-left manage-head-con">轮播管理
          ...
          <div class="authority-content">
            <div class="list-content show">
              <div class="offcial-table tr-border margin-big-top clearfix">
                <div class="tr-th clearfix">
                  <div id="controller" style="width: 100%;">
                    ...
                    <div class="fr" style="padding-left: 5px;">
                      ...
                      <a class="button btn-red text-white" v-on:click="resetCurPageOrder(),baseInfo(0,1)">时间降序</a>
                    </div>
                    <div class="fr" style="padding-left: 5px;">
                      <router-link :to="{name:'uploadRotation'}" class="button btn-orange text-white">添加</router-link>
                    </div>
                    <div class="fr" style="padding-left: 5px;">
                      <input type="search" placeholder="id/账号只能数字"
                             style="border:solid 1px #00a5e0; height: 25px;width: 100px;" ref="search"
                             v-on:input="searchValue">
                      <a class="button bg-green-deep" v-on:click="resetCurPageSearch(),baseInfo(search,0)">查询</a>
                    </div>
                  </div>
                </div>
                <div class="tr-th clearfix">
                ...
                <div class="tr clearfix border-bottom-none" v-for="item in Info.rotationList">
                  <div class="td w20">
                    {{item.id}}
                  </div>
                  <div class="td w20">
                  ...
                  <div class="td w15">
                    <a class="button btn-red text-white" v-if="item.state==1" v-on:click="sheildRotation(item.id,0)">禁用</a>
                    <a class="button bg-green-deep" v-if="item.state==0" v-on:click="sheildRotation(item.id,1)">启动</a>
                  </div>
                </div>
              </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            baseInfo: function (id, flag) {
                ...
                that.$axios({
                    method: "post",
                    url: "http://47.102.97.234:9090/queryRotation",
                    data: {
                        id: id,
                        flag: flag,
                        curPage: curPage
            ...
            sheildRotation: function (id, state) {
                var that = this;
                var id = id;
                var state = state
                that.$axios({
                    method: "post",
                    url: "http://47.102.97.234:9090/sheildRotation",
                    data: {
                        id: id,
                        state: state
                    },
